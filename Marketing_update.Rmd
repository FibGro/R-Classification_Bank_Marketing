...
Now, we create a decision tree model by using `rpart()`. 

```{r}
set.seed(100)

# Create a decision tree model 
model_dt <- rpart(formula = y ~., data = bank_train_up, method = "class")
```


## Interpretation 

We can visualize decision tree by using function `fancyRpartPlot()`. Then, we observe the important variables in the model by using function `vip()`

```{r}
# Visualize the decision tree
fancyRpartPlot(palettes = "Reds", model_dt, type=4, sub = NULL)

# Importance Variable
vip(model_dt, num_features = 10, scale = TRUE, mapping = aes_string(fill = "Variable"), aesthetics = list(fill = "#007200"))

```

> Insight : 
>
- Based on the decision tree, the poutcome becomes the root of a tree, implying that this variable has the highest information gain. 
- The model is considered simple with five interior nodes and seven terminal nodes. 
- The potcome, month, contact and age are the most important predictors of y.
...
prob_dt <- predict(object = model_dt, newdata=bank_test, type="prob")
...
> Insights:
>
- The accuracy value of 0.7954, impliying that the model can predict 79.54% of the test data correctly.
- The precision value shows 0.2938, implying that 29.38% of the correctly predicted cases turned out to be positive. It also means that the model is bad at classifying the positive predicted cases.
- The sensitivity and specificity values are 53.94% and 82.92%, respectively. It implies that most actual negative classes are correctly classified, as opposed to the positive class.
...
pred_dt_train <- predict(object=model_dt, newdata=bank_train_up, type="class")
...
```
> Insight: As illustrated in the plot, the intersection between specificity and sensitivity at 0.4 has a similar performance metrics value with model with threshold at 0.5. 
...
> Insight: 
>
- The summary table shows that the model with Random Forest using K-fold cross-validation produces the highest values of accuracy. It implies that this model is appropriate to be used to correctly predict the customer's preference toward the term deposit (product). 
- Since the model with Random Forest also has the highest value of precision, this model can be used to reduce the labour cost by avoiding calling uninterested customers. 
- However, if the objective of the bank is to increase the number of customers and does not want to lose any opportunities, the model with Naive Bayes is the appropriate model to be used. It is confirmed by the highest value of sensitivity.
- To be concluded, the model with Random Forest with K-fold cross-validation is the best model for our project objective, confirmed by the highest value of accuracy and AUC. 
